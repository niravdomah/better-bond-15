...
This is the **canonical quality gate run** for each story. The developer agent runs only `npm test` during IMPLEMENT to verify its own work — the full suite (security, code quality, testing, performance) runs here. All gate results from this run are authoritative.
