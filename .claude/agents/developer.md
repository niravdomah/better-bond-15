...
The orchestrator invokes you in a single autonomous call. The parent does not run `npm test` before invoking you — you capture the baseline yourself, implement, then re-verify.

**Single call — Implement + Verify:**

1. Run `npm test --run` first to capture the failing-test baseline. Record the pass/fail counts.
2. Read the story file (resolve from the glob the orchestrator passes) and the test files generated by `test-generator`. Read the test-handoff document if the glob resolves to one.
3. Write code to make all failing tests pass.
4. Re-run `npm test --run` to verify. Fix any remaining failures.
5. Return a summary that includes: initial baseline (pass/fail counts), what was implemented, final pass/fail counts, any unresolved issues for QA.

Do NOT run lint, build, `test:quality`, Playwright, or any other quality gate — the code-reviewer runs the full canonical suite during QA. Do NOT commit.
...
6. `{ content: "    >> Verify tests pass", activeForm: "    >> Verifying tests pass" }`
...
**Placement in the flow:** This phase runs after all tests pass and linting is clean, but before returning to the orchestrator — i.e., between Phase 2 (Implementation) and Phase 3 (Test Verification).
...
**Run `npm test --run`** to verify all tests pass after implementation. Fix any failures.

This is a fast verification step — the code-reviewer runs the full canonical quality gate suite (lint, build, test:quality, security) during the QA phase. Running just tests here catches the most likely regression (broken tests) without duplicating the full gate run.
...
The developer agent runs `npm test` to verify tests pass before transitioning to QA. The full quality gate suite (lint, build, test:quality, security) runs during the QA phase via code-reviewer.
