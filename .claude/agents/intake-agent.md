...
> **`backendConnectivity` note:** This field is populated by the `api-connectivity-agent` (orchestrated as Step 4b of `/start`), not by `intake-agent`. Always initialise it as `null` in Call B; the connectivity agent will overwrite it after it runs. See the "backendConnectivity Field" section below for the schema.
...
| `context.backendConnectivity` | `null` or `BackendConnectivity` object (schema below) | Populated by the `api-connectivity-agent` after Call B. Always initialise as `null` here. |
...
#### `backendConnectivity` Field (populated post-intake by `api-connectivity-agent`)

Set to `null` in Call B. The orchestrator's Step 4b (`api-connectivity-agent`) overwrites this block with one of three shapes after running:

**Shape 1 — Skipped (no backend or backend not yet running):**
```json
"backendConnectivity": {
  "skipped": true,
  "reason": "dataSource=mock-only" | "dataSource=new-api (no backend yet)" | "backend not yet running"
}
```

**Shape 2 — Verified (smoke test passed):**
```json
"backendConnectivity": {
  "skipped": false,
  "sourceMethod": "spec" | "user_curl",
  "baseUrl": "https://api.example.com",
  "authScheme": "bearer" | "apiKey" | "basic" | "oauth2-client-creds" | "cookie" | "none" | "custom",
  ...
  "smokeTest": {
    ...
    "method": "GET",
    "verifiedStatus": 200,
    "verifiedAt": "2026-05-06T19:30:00Z",
    "curlArtifact": "generated-docs/context/api-smoke-test.sh"
  },
  "userCurlExample": "curl -H 'Authorization: Bearer ${API_TOKEN}' https://api.example.com/v1/users/me",
  "notes": "CORS allowed for localhost:3000 — no proxy needed."
}
```

**Shape 3 — Captured but unverified (3-attempt cap hit, user proceeded anyway):**
```json
"backendConnectivity": {
  "skipped": false,
  "sourceMethod": "spec" | "user_curl",
  ...
  "smokeTest": {
    ...
    "verifiedAt": null,
    "curlArtifact": "generated-docs/context/api-smoke-test.sh",
    "lastError": "401 Unauthorized — token format may be wrong"
  },
  "userCurlExample": "curl -H 'Authorization: Bearer ${API_TOKEN}' https://api.example.com/v1/users/me",
  "warning": "Smoke test failed after 3 attempts; user opted to proceed. Re-run via /api-status --check after fixing.",
  "notes": "..."
}
```

> `sourceMethod` and `userCurlExample` are present whenever the agent ran (Shape 2 or Shape 3). `userCurlExample` is **optional** in Shape 2 — present only when `sourceMethod: "user_curl"`, omitted when `sourceMethod: "spec"`.

**Field semantics:**

| Field | Type | Description |
|-------|------|-------------|
| `skipped` | `boolean` | `true` only when dataSource makes the check non-applicable. Otherwise `false` |
| `sourceMethod` | `"spec" \| "user_curl"` | How the smoke test was derived. `"spec"` = parsed from the OpenAPI spec; `"user_curl"` = parsed from a user-pasted curl example (the curl-fallback path). Present whenever the agent ran (Shape 2/3); absent in Shape 1 |
| `baseUrl` | `string` | The verified base URL — usually mirrors `NEXT_PUBLIC_API_BASE_URL` in `web/.env.local` |
| `authScheme` | `string` | Normalised auth scheme. Use `"none"` for public APIs |
| `authHeader` | `string` or `null` | The HTTP header name carrying credentials. `null` when `authScheme: "none"` |
| `authValueFormat` | `string` or `null` | Template for the header value. `{token}` is the placeholder. Examples: `"Bearer {token}"`, `"{token}"` (raw key) |
| `credentialEnvVars` | `string[]` | The env var **names** (not values) that hold credentials. Values live only in `web/.env.local`. When `sourceMethod: "user_curl"`, this list also includes any env vars the agent created during auto-redaction of literal credentials in the pasted curl |
| `smokeTest.endpoint` | `string` | Path used to verify connectivity |
| `smokeTest.method` | `string` | Usually `GET` |
| `smokeTest.verifiedStatus` | `number` or `null` | The HTTP status received. `null` when the test never succeeded |
| `smokeTest.verifiedAt` | ISO 8601 string or `null` | When the verification last passed. `null` when never verified |
| `smokeTest.curlArtifact` | `string` | Path to the re-runnable `.sh` script (no credentials embedded — uses env vars) |
| `smokeTest.lastError` | `string` (optional) | Present only on Shape 3 — short remediation hint from the last attempt |
| `userCurlExample` | `string` (optional) | Present only when `sourceMethod: "user_curl"`. The **redacted** curl (literal credentials replaced by `${VAR_NAME}` placeholders). Useful for re-running the smoke test outside the workflow and for downstream agents to see the exact contract that worked |
| `warning` | `string` (optional) | Present only on Shape 3 — describes the deferred verification |
| `notes` | `string` (optional) | Free-text observations (CORS, proxy needs, VPN, etc.) |

**Security:** Credential VALUES never appear in this manifest. Only env var NAMES (in `credentialEnvVars`) and placeholder-only curl strings (in `userCurlExample`). When the agent reads `userCurlExample` from a previous run or writes a new one, any `Authorization: Bearer ...` substring containing characters other than `${...}` placeholders is a contract violation — the agent must refuse to write it.
