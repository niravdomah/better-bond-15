...
- `generated-docs/context/intake-manifest.json` — what exists, what's needed, basic context (project description, roles, data source, styling), and **`context.backendConnectivity`** populated by the `api-connectivity-agent` that ran in Step 4b (see "Backend connectivity awareness" below)
...
| 6 | Backend Integration | Mirror of `context.backendConnectivity` from the manifest. **Do not re-ask** — populate from the manifest. See "Backend connectivity awareness" below |
| 7 | Key Workflows | Step-by-step user flows? Happy path and error paths? |
| 8 | Compliance & Regulatory | See compliance screening below — only flag domains identified during INTAKE |
| 9 | Non-Functional Requirements | Accessibility, performance, responsive behavior? **Add candidate NFRs from `backendConnectivity` findings (CORS, VPN, etc.)** |
...
**Backend connectivity awareness:** When `context.backendConnectivity` is present in the manifest, the connectivity agent already attempted a real round-trip with the user's credentials. Translate its findings into FRS-shaped requirements rather than asking the user to repeat the answers:

| `backendConnectivity` signal | What to do in gap analysis |
|---|---|
| Shape 1 (`skipped: true`) | No action — backend integration isn't part of this project. The Backend Integration section in the FRS will read "No backend integration — see manifest dataSource." |
| Shape 2 (`smokeTest.verifiedStatus` is 2xx) | Mirror baseUrl/authScheme/authHeader/authValueFormat/credentialEnvVars/smokeTest into the FRS's Backend Integration table. **Do NOT ask the user to confirm** — they already approved this in Step 4b. |
| Shape 2 with `cors_warning: true` (or `notes` mentioning CORS) | Generate a candidate NFR — e.g., "NFR: Frontend API calls from `localhost:3000` MUST go through a Next.js rewrite proxy (CORS headers absent on backend)." Ask the user only if they want to phrase it differently. |
| Shape 3 (`smokeTest.verifiedStatus: null`, `warning` set) | Mirror what was captured into the Backend Integration section but mark `verifiedStatus` as `null (deferred)` and include the warning verbatim. Ask the user **one** question: "The smoke test couldn't be verified — should we proceed assuming this config is correct, or revise anything before the FRS is written?" |
| `smokeTest.lastError` contains "404" on a spec-declared endpoint | Flag a `dataSource` reclassification question — e.g., "The smoke test 404'd on `[endpoint]` even though the spec lists it. Has the backend not implemented this endpoint yet? If so, `dataSource` should change from `existing-api` to `api-in-development` (which scaffolds mocks)." |
| Auth-scheme mismatch — `backendConnectivity.authScheme` differs from what `context.authMethod` implies | Ask: "The connectivity check found `[scheme]` auth on the backend, but you chose `[authMethod]` for user-facing auth. Are these the same flow, or do they layer (e.g., BFF cookie + bearer token to upstream API)?" |
| `notes` mentions VPN, internal network, or proxy | Generate a candidate NFR — e.g., "NFR: Development environment requires VPN access to backend host." |

When generating these candidate NFRs/CRs from connectivity findings, mark the `Source` in the traceability table as `intake-manifest.json (context.backendConnectivity)` rather than "User input."
...
5. **Use the template structure exactly** — All 10 sections must appear. Mark N/A when confirmed by user.
...
- [ ] All 10 FRS template sections addressed (filled or flagged for N/A confirmation)
