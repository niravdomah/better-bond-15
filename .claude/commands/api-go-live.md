...
- **Re-runs the captured smoke test** (from INTAKE Step 4b) against the live backend before flipping anything. By default, refuses to flip if the smoke test fails.
...
|---|---|
| **Missing** (legacy project — INTAKE ran before this gate existed) | Output: "No connectivity capture found in the manifest — going live without verification. (Re-run /start on a fresh project to capture connectivity in INTAKE Step 4b.)" Skip to Step 2. |
