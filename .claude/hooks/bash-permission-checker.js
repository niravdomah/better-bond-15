...
const cdPrefix = '(?:cd\\s+["\']?' + absPathChar + '+["\']?\\s*&&\\s*)?';
...
const npmPrefix = '(?:--prefix\\s+["\']?' + absPathChar + '+["\']?\\s+)?';
...
let allowPatterns = [
  ...
  cdEnvPrefix + 'npx\\s+playwright(?:\\s+.*)?$',
  ...
  cdPrefix + 'find\\s+["\']?' + absPathChar + '+["\']?(?:\\s+(?:-(?:name|iname|type|maxdepth|mindepth|path)\\s+["\']?[\\w.*?/\\\\:-]+["\']?|-(?:empty|print0?)|!|-not|-o|\\\\[()]?))*\\s*$',
  ...
  '(?:test\\s+-[defrsxw]|\\[\\s+-[defrsxw])\\s+["\']?' + absPathChar + '+["\']?(?:\\s+-[oa]\\s+-[defrsxw]\\s+["\']?' + absPathChar + '+["\']?)*\\s*\\]?\\s*$',
  'true\\s*$',
  'false\\s*$',
