...
| After IMPLEMENT | After developer call returns | Fire-and-forget inline |
...
| developer | IMPLEMENT | 1 (single, autonomous) | 0 (fully autonomous) |
...
### IMPLEMENT (single call — fully autonomous)

The orchestrator does NOT run `npm test` from the parent — the developer agent runs it internally as its first step (after `--mark-started`) and reports the baseline back. Pre-reading the story or test-handoff from the parent is also unnecessary; the agent reads them itself. Pass file globs (not resolved paths) and let the agent resolve.

**Single call — Implement + Verify:**

> "This is the IMPLEMENT call for Epic [N], Story [M]. Story file glob: `generated-docs/stories/epic-[N]-*/story-[M]-*.md`. Test handoff glob: `generated-docs/test-design/epic-[N]-*/story-[M]-*-test-handoff.md`. Run `npm test --run` first to capture the failing-test baseline, then write code to make all failing tests pass. Re-run `npm test --run` to verify and fix any remaining failures. Do NOT run lint, build, or test:quality — the code-reviewer runs the full canonical quality gate suite during QA. Do NOT commit. Return a summary that includes the initial baseline (pass/fail counts), what was implemented, and the final pass/fail counts."

**Dashboard update (fire-and-forget):** After the developer agent returns, regenerate the HTML dashboard per the [Dashboard Update Policy](#dashboard-update-policy).

After it returns, proceed directly to QA.

**Why a single call:** Both halves of the prior 2-call split (`Implement` then `Pre-flight Test Check`) were fully autonomous with no user gate between them. Splitting forced extra parent tool calls (separate Agent dispatches plus the parent's own pre-flight `npm test`) that consumed the per-response PreToolUse-hook budget without adding observability the user could see. The merged call returns the same data in the agent's summary.
...
After Call B returns, but before the user is asked to verify in the browser, Playwright runs against a live dev server to catch runtime issues. **This step never asks the user on pass — only on escalation after 3 failed auto-fix cycles.**

**The orchestrator does NOT run Playwright directly.** The bash sequence (Glob spec resolution, Grep for `test.fixme`, Bash for Playwright, Read for the JSON report) exceeds the parent's per-response tool-call budget and silently breaks PreToolUse hook dispatch. Delegate to the `playwright-runner` agent instead.

1. **Build the input from Call B's output:**
   - Parse `Route:` — `<path>` or `N/A`.
   - Parse `Deferred stories:` — list of `(epic-N, story-M)` pairs.
   - Compute `currentTargetGlob = web/e2e/epic-<N>-story-<M>-*.spec.ts` and `deferredTargetGlobs` (one glob per deferred story, possibly empty).

2. **Launch `playwright-runner`:**
   > "Run E2E Verification for Epic [N], Story [M]. `route`: `<path>` or `N/A`. `currentTargetGlob`: `<glob>`. `deferredTargetGlobs`: [list]. `fixCycleCount`: <current `e2eFixCycleCount` from state, 0 on first invocation>. Follow the diagnostic in your agent definition and return a single JSON object."

3. **Parse the agent's JSON return** and branch on `status`. The agent has already recorded the appropriate `e2eStatus` to workflow state and fired the dashboard update — do NOT re-record from the parent.
   - `auto-skipped:non-routable` → proceed to the non-routable manual-verification branch below (skip browser checklist).
   - `auto-skipped:fixme` → proceed to manual verification (deferred items still get checked manually).
   - `halt` → use `AskUserQuestion` with the three-way Halt prompt below. The agent's `targets` array tells you which spec triggered the halt and why. The agent did **not** record state on halt; the parent records it after the user resolves the prompt (see Halt branch state recording below).
   - `passed` → display the agent's `summary` line and proceed to manual verification. (The agent recorded `passed` for first runs, `passed-after-fix` if `fixCycleCount > 0`.)
   - `failed` → auto-trigger the **E2E fix cycle** below using the agent's `failures` array as the issues payload — do NOT ask the user.
...
**Halt branch — state recording (parent-owned):** The only `--set-e2e-status` case the parent records — for `passed`, `passed-after-fix`, and `failed`, playwright-runner already did. After the user picks:

- (a) Re-run test-generator → no recording yet; the next playwright-runner invocation records.
- (b) Mark `test.fixme()` → record `auto-skipped:fixme`.
- (c) Skip this round → record `user-skipped`.

```bash
node .claude/scripts/transition-phase.js --set-e2e-status <status> --current --story M
```
...
The parent orchestrator runs the fix cycle directly — launches the developer agent, then re-runs E2E Verification. Quality gates re-run once at the end (Call C), not inside the fix cycle.

**Two triggers:**

- **User-triggered:** user picked "Issues found" during manual verification.
- **E2E-triggered:** Playwright failed during E2E Verification. No user prompt — the Playwright JSON report is the issues payload.

**Flow:**

1. (User-triggered only) `AskUserQuestion`: "What did you find?" → fresh turn.
2. Launch developer:
   > "Fix these issues in Epic [N], Story [M]: [user's description OR the playwright-runner `failures` array, which lists each failing test's title, error, filePath, and tracePath]. [E2E-triggered: Playwright traces are under `web/test-results/` if you need to inspect them.] Do NOT run quality gates, Vitest, or Playwright. Do NOT commit. Do NOT use AskUserQuestion."
3. Re-run **E2E Verification** against the same combined target set, passing the current `e2eFixCycleCount` so playwright-runner records `passed-after-fix` rather than `passed` on success.
4. **If pass:**
   - User-triggered → display fix summary + full checklist (verbatim from `generated-docs/qa/epic-N-[slug]/story-M-[slug]-verification-checklist.md`), `AskUserQuestion` with the three options.
   - E2E-triggered → proceed to manual verification with note "Fixed N failing tests; please verify in the browser." (`e2eStatus: passed-after-fix` was already recorded by the agent.)
5. **If fail:** increment `e2eFixCycleCount`, loop from step 2. **Cap at 3 cycles.** On the 4th attempt, record `e2eStatus: escalated` and `AskUserQuestion`:
   - "Fix manually" — exit the loop.
   - "Skip E2E this round" — record `e2eStatus: user-skipped-after-escalation` and proceed.
   - "Mark failing specs `test.fixme()`" — launch developer to wrap failing `test()` blocks in `test.fixme()` with an escalation comment, then proceed.

**Always re-present the full manual verification checklist** every time the user is asked to verify or re-verify. Read it verbatim from the checklist file each time — never abbreviate, never refer the user to a previous message.

After resolution: user picks "Issues found" again → repeat (fresh turn each cycle). Otherwise → proceed to spec compliance check, noting the fix cycle occurred.
...
2. After developer returns, display summary and re-run watchdog Call A to verify the inconsistencies are resolved.
...
4. When watchdog passes → proceed to Call C (code was changed, so Call C must re-run quality gates).
...
```jsonc
{
  ...
  "epicPass": {
    ...
    "storyPhases": {                        // per-story state within the current pass
      "1": "COMPLETE",
      "2": "IN_PROGRESS",
      "3": "PENDING"
    },
    "passStartedAt": "2026-05-12T11:42:01Z",
    "fixCycles": {},                        // per-story fix counter, reset each pass
    "qaFindings": {}                        // populated during EPIC-QA only
  }
}
```
...
### Pass-by-pass execution model

Each pass below corresponds to a `### Phase: EPIC-<PASS>` section in [continue.md](../commands/continue.md).

**EPIC-REALIGN**

- Run REALIGN Call A autonomously for every story in `storyOrder`. Auto-complete (via `--complete-story-pass`) any story whose Call A reports `impactsFound: false`.
- After every story has produced its impacts proposal, present them in a SINGLE batched `AskUserQuestion` round, grouped by story (max 4 questions per call).
- For each story whose impacts are approved, run REALIGN Call B to apply changes, then call `--complete-story-pass --story M`.
- When `storyPhases` are all COMPLETE, call `--advance-pass`. Dashboard update.

**EPIC-TEST-DESIGN**

- Run test-designer Call A for every story. Enumerate BA decisions via `list-ba-decisions.js` and BUFFER them in memory keyed by story — do NOT prompt yet.
- Present BA decisions in batched `AskUserQuestion` rounds — grouped by story, MAX 3 decisions per call. Continue rounds until every decision is resolved. Persist via `resolve-ba-decision.js`.
- Run test-designer Call B per story to rewrite the test-design doc with resolved decisions AND emit the `renderScope` field in `test-handoff.md`. Call `--complete-story-pass --story M` after each.
- `--advance-pass` when complete. Dashboard update.

**EPIC-WRITE-TESTS**

- Sequential, fully autonomous. For each story: launch test-generator (single call), then `--complete-story-pass --story M`.
...
The single QA pass for the entire epic. One E2E run, one batched manual verification, one commit.
...
| Risk | Mitigation |
|------|-----------|
| Cascade regressions from per-story fixes during EPIC-QA | Full-epic Vitest + Playwright + spec-compliance gate runs AFTER all per-story fixes land. Non-negotiable. |
| Single big commit rollback | `pre-epic-N-batched` git tag at upgrade time + optional per-pass checkpoint tags. `git reset --hard pre-epic-N-batched` recovers cleanly. |
| Late test-design discovery (a wrong assumption in story 3 surfaces during IMPLEMENT after story 1+2 are already approved) | `epicPass.storyPhases[N] = "REWORK"` allows narrow re-entry into TEST-DESIGN for that story without restarting the pass. |
| Unresumable state from `epicPass` corruption | `--repair` reconstructs `epicPass` from disk artifacts (test-design files, test files, committed code). |